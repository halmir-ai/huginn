...
const frozenPaths = [`src/games/${game}`, `tests/${game}.test.ts`, "src/game-runtime", "src/vite-env.d.ts", "LICENSE", "package.json", "package-lock.json", "tsconfig.json", "docs/demo/ARCADE_FEATURE_PROTOCOL.md", ...(treatment === "huginn" ? ["src/debugger", "src/huginn", "src/webmcp"] : [])];
...
for (const path of [`src/games/${game}`, "src/game-runtime", "src/vite-env.d.ts", `tests/${game}.test.ts`, "LICENSE"]) await copyPath(path);
try { await access(join(repo, `public/assets/${game}`)); await copyPath(`public/assets/${game}`); }
catch (error) { if (error.code !== "ENOENT") throw error; }
if (treatment === "huginn") for (const path of ["src/debugger", "src/huginn", "src/webmcp"]) await copyPath(path);
const main = `import { ${game}Game } from './games/${game}/game';\nimport { ${mount} } from './games/${game}/view';\nimport { GameRuntime } from './game-runtime';\n${treatment === "huginn" ? "import { attachHuginnDebugger } from './debugger';\n" : ""}const runtime = new GameRuntime(${game}Game, 12);\n${mount}(document.querySelector('#app')!, runtime, { assets: './assets/${game}' });\n${treatment === "huginn" ? "await attachHuginnDebugger(runtime, document.querySelector('#experiment')!);\n" : ""}`;
...
const manifest = { format: "huginn/feature-baseline-v1", game, treatment, sourceCommit, seed: 12, gameFiles: files, gameSourceDigest: createHash("sha256").update(JSON.stringify(files)).digest("hex"), sharedRuntimeSha256: createHash("sha256").update(await readFile(join(destination, "src/game-runtime/index.ts"))).digest("hex"), tokens: null, status: "exported-not-started" };
