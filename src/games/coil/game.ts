...
export type CoilAction = { type: "turn"; direction: Direction } | { type: "advance"; steps: 1 | 5 | 10 };
...
function initialState(seed: number): CoilState {
  ...
  return { version: 1, seed, rng, phase: "playing", snake, direction: "e", pendingDirection: null, food, bonus: null, foodsEaten: 0, bonusesEaten: 0, score: 0, tick: 0, death: null };
}
...
function legalActions(state: CoilState): LegalAction<CoilAction>[] {
  ...
  for (const steps of [1, 5, 10] as const) actions.push({ action: { type: "advance", steps }, label: `Advance ${steps} cell${steps === 1 ? "" : "s"}`, reason: "Move straight by fixed cells; stop immediately on collision." });
  return actions;
}
...
function advance(state: CoilState, events: CoilEvent[]): CoilState {
  ...
  let next: CoilState = { ...state, snake: [head, ...body], direction, pendingDirection: null, tick: state.tick + 1 };
  if (next.bonus) {
...
function deserialize(value: unknown): CoilState {
  ...
  if (!exact(value, ["version", "seed", "rng", "phase", "snake", "direction", "pendingDirection", "food", "bonus", "foodsEaten", "bonusesEaten", "score", "tick", "death"])) return invalid();
...
export const coilGame: GameDefinition<CoilState, CoilAction, CoilEvent> = {
  description: {
    id: "coil", title: "COIL", version: "1.0.0",
    summary: "After-hours Snake. Stay alive, grow your coil, and risk the shortcut for gold.",
    rules: ["Each advance moves one or more fixed cells. Turns take effect on the next cell; never reverse.", "Coral fruit grows the snake by one cell and earns 10 points.", "Every five coral fruits increases speed and reveals a golden fruit worth 50 points for 52 cell steps.", "Golden fruit does not grow the snake. Touching a wall or your own body ends the run.", "There is no score quota: chase your personal best. Pausing and animation are outside the simulation."],
    victoryConditions: [], failureConditions: ["The head enters a wall or an occupied body cell, except a tail vacating on that same step."],
    metrics: [
    ...
    actions: [
      ...
      { type: "advance", description: "Advance 1, 5, or 10 fixed cells in the chosen direction, stopping at death.", inputSchema: { type: "object", properties: { type: { const: "advance" }, steps: { type: "integer", enum: [1, 5, 10] } }, required: ["type", "steps"], additionalProperties: false } },
    ],
  },
  initialState,
  legalActions,
  reduce(state, action) {
    ...
    let next = state;
    for (let step = 0; step < action.steps && next.phase === "playing"; step++) next = advance(next, events);
    return { state: next, events };
  },
  metrics(state) {
    return { score: state.score, length: state.snake.length, foodsEaten: state.foodsEaten, bonusesEaten: state.bonusesEaten, level: levelFor(state), stepDurationMs: stepDurationMs(state), bonusStepsLeft: state.bonus?.remaining ?? 0, tick: state.tick, alive: state.phase === "playing" };
  },
  deserialize,
};
