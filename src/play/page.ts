...
export function gamePage(gameId: "coil" | "starfall" | "thornwatch", plain = false) {
  ...
  nav.innerHTML = `<a class="brand" href="${href("")}">HUGINN / ARCADE</a><div class="nav-links"><a href="${href("games/coil/")}">COIL · Snake</a><a href="${href("games/starfall/")}">STARFALL · Pinball</a><a href="${href("games/thornwatch/")}">THORNWATCH · Defense</a><a href="${href(`games/${gameId}/${plain ? "" : "plain/"}`)}">${plain ? "With Huginn" : "Standalone game"}</a><a href="https://github.com/halmir-ai/huginn">Source ↗</a></div>`;
  app.before(nav);
